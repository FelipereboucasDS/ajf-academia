...
// Table: app_settings
//   id: integer (not null)
//   booking_cutoff_time: time without time zone (not null, default: '22:00:00'::time without time zone)
//   billing_cutoff_type: text (not null, default: 'last_business_day'::text)
...
//   FOREIGN KEY invoices_student_id_fkey: FOREIGN KEY (student_id) REFERENCES profiles(id) ON DELETE CASCADE
//   UNIQUE invoices_student_id_period_key: UNIQUE (student_id, period)
// Table: profiles
...
// Table: app_settings
//   Policy "admin_update_settings" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
//   Policy "public_read_settings" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: bookings
...
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
//   Policy "student_insert_invoices" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (student_id = auth.uid())
//   Policy "student_own_invoices" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (student_id = auth.uid())
//   Policy "student_update_invoices" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (student_id = auth.uid())
//     WITH CHECK: (student_id = auth.uid())
// Table: profiles
...
//     USING: (id = auth.uid())
// Table: schedules
//   Policy "admin_delete_schedules" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
//   Policy "admin_insert_schedules" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
//   Policy "admin_update_schedules" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
//   Policy "public_schedules" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: units
//   Policy "admin_delete_units" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
//   Policy "admin_insert_units" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
//   Policy "admin_update_units" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
//     WITH CHECK: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text))))
...
// --- INDEXES ---
// Table: bookings
//   CREATE UNIQUE INDEX one_booking_per_day_idx ON public.bookings USING btree (student_id, date) WHERE (status = 'booked'::text)
// Table: invoices
//   CREATE UNIQUE INDEX invoices_student_id_period_key ON public.invoices USING btree (student_id, period)
